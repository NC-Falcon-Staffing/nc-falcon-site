...
**There is no Save button.** Decap only splits Save from Publish under
`publish_mode: editorial_workflow`, which we don't use. On every screen the
**Publish** button is the only commit action, and on ordinary page screens it
means nothing more than "save my work". The gate decides whether a commit
becomes a deploy, not the button.
...
That is why the marker collection is called **GO LIVE** rather than "Publish
Site". "Press Publish to publish" would be indistinguishable from "press
Publish to save". Don't rename it back.
